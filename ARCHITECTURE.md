...
### Optional OAuth 2.1 resource server on `/mcp`

The MCP port defaults to open — the mcp-tunnel + NetworkPolicy are its only guard. Setting `MCP_AUTH=workos` (plus `WORKOS_AUTHKIT_ISSUER` + `MCP_PUBLIC_URL`) turns `/mcp` into an OAuth 2.1 **resource server** (`src/mcp/oauth.ts`), delegating authorization to **WorkOS AuthKit** as the authorization server — which is what lets the surface be added directly as a Claude custom connector over a public tunnel. The resource-server half owes three things: it serves Protected Resource Metadata (RFC 9728) at `/.well-known/oauth-protected-resource`; it enforces a `Bearer` token on `/mcp`, answering failures with the RFC 9728 §5.1 `WWW-Authenticate: Bearer resource_metadata="…"` challenge (401 missing/invalid, 400 malformed, 403 insufficient scope); and it validates the token with `jose` — signature against the WorkOS JWKS, `iss`, `exp`/`nbf`, and critically `aud` equal to this server's canonical URI (`MCP_PUBLIC_URL`). That last check is the RFC 8707 binding: a token minted for another resource is rejected outright, so a connector cannot replay a token across tenants. The token is never forwarded to Bedrock — Bedrock authenticates with its own IRSA credential; OAuth only decides whether the caller reaches the tools. WorkOS is configured entirely in its dashboard (DCR/CIMD, resource indicators, scopes); the module holds no client secret (it verifies against the public JWKS) and is written self-contained so it can later be lifted into the shared runtime library for other tenants. Full operator + connector + self-tunnel steps: [`docs/mcp-oauth.md`](docs/mcp-oauth.md).
