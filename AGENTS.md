...
| `prometheusrule.yaml`    | Alert rules — crawl failures, circuit-breaker open, alert-send failures, pgvector unreachable. Off by default: they need a Prometheus Operator ruler, which eks-gitops (OpenTelemetry Collector → AMP) does not run. Enable on `kx`                    |
