...
Observability is cluster-level via eks-gitops: app stderr → log forwarder → Grafana Cloud Loki; OTLP traces + metrics → `otel-collector.observability.svc.cluster.local:4318` → Grafana Cloud Tempo + Mimir. No per-pod sidecars. Required resource attrs `agents.tenant=protohype` + `agents.platform=competitive-intelligence` ride on every span/metric. See `chart/README.md` for the full template-by-template description.
